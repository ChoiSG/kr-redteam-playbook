...
이런 계정 정보 탈취 형식의 피싱 공격은 다중 인증 (Multi-Factor Authentication) 이 나오면서 어느정도 대처가 됐다. 첫번째로 공격자가 피해자 유저의 핸드폰이나 유비키 등을 물리적으로 손에 넣지 않는 한 두번째 인증을 통과하기가 불가능 하기 때문이고, 두번째로는 공격자의 웹사이트가 이중/다중 인증까지 구현하기는 어려웠기 때문이다.&#x20;
...
피싱일까? 잘 모르겠지만 어쨌든 이중 인증을 설정해놨으니 상관 없다. 다중 인증은 무이라고 배웠다. 한번 가서 재밌는 MBTI 관련 정보를 받아봐야겠다.&#x20;
...
이 와중에 공격자는 계정 정보와 이중인증이 들어간 세션 쿠키 탈취했다.&#x20;
